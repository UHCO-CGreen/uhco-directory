...
<cfif request.getSettingsFeatureAvailability("migrations").isDisabled>
    <cfset errorMsg = request.getSettingsFeatureAvailability("migrations").reason>
    <cfif returnJson OR triggeredBy EQ "scheduled">
        <cfcontent type="application/json; charset=utf-8">
        <cfoutput>#serializeJSON({ success:false, disabled:true, message:errorMsg })#</cfoutput>
        <cfabort>
    </cfif>
    <cflocation url="#request.webRoot#/admin/settings/?disabledFeature=migrations" addtoken="false">
</cfif>

<cfif triggeredBy NEQ "scheduled" AND NOT request.hasPermission("migrations.manage")>
...
<cftry>
    ...
    <cfif triggeredBy EQ "scheduled" AND NOT migrationService.isAutoExecuteEnabled()>
        ...
        <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm?msg=skipped&err=#urlEncodedFormat(errorMsg)#" addtoken="false">
    </cfif>
...
<cfif success>
    <cfif structKeyExists(result, "totalErrors") AND val(result.totalErrors) GT 0>
        <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm?msg=ran_with_errors&runID=#result.runID#&errors=#val(result.totalErrors)#" addtoken="false">
    <cfelse>
        <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm?msg=ran&runID=#result.runID#" addtoken="false">
    </cfif>
<cfelse>
    <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm?msg=error&err=#urlEncodedFormat(errorMsg)#" addtoken="false">
</cfif>
