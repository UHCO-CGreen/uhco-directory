...
<cfif runID EQ 0>
    <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm" addtoken="false">
</cfif>
...
<cfif structIsEmpty(run) AND dbOk>
    <cflocation url="#request.webRoot#/admin/migrations/grad_migration.cfm?msg=error&err=#urlEncodedFormat('Run not found.')#" addtoken="false">
</cfif>
...
<!--- Sort params for user details --->
<cfset allowedDetailSortCols = ["LASTNAME","FIRSTNAME","STATUS"]>
<cfset sortCol = (structKeyExists(url, "sortCol") AND arrayFindNoCase(allowedDetailSortCols, url.sortCol) GT 0) ? url.sortCol : "LASTNAME">
<cfset sortDir = ((url.sortDir ?: "") EQ "DESC") ? "DESC" : "ASC">
<cfscript>
    arraySort(details, function(a, b) {
    ...
    function detailSortLink(col) {
        var dir = (sortCol EQ col AND sortDir EQ "ASC") ? "DESC" : "ASC";
        return "?runID=" & runID & "&sortCol=" & col & "&sortDir=" & dir;
    }
...
<nav aria-label="breadcrumb">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/admin/migrations/grad_migration.cfm">Graduation Migration</a></li>
        ...
        <cfelse>
            <div class="table-responsive">
                <table class="table table-sm table-hover align-middle mb-0 settings-table">
                    <thead>
                        <tr>
                            <th><a href="#detailSortLink('LASTNAME')#" class="settings-sort-link">Name#detailSortArrow('LASTNAME')#</a></th>
                            <th>Email</th>
                            <th>Previous Title</th>
                            <th>New Title</th>
                            <th>Exclusions +/-</th>
                            <th><a href="#detailSortLink('STATUS')#" class="settings-sort-link">Status#detailSortArrow('STATUS')#</a></th>
                            <th>Error</th>
                        </tr>
                    </thead>
                    <tbody>
