...
<!--- ── URL params ── --->
<cfif NOT request.hasPermission("migrations.manage")>
    <cflocation url="#request.webRoot#/admin/unauthorized.cfm" addtoken="false">
</cfif>

<cfinclude template="/admin/settings/feature-gates.cfm">
<cfif request.getSettingsFeatureAvailability("migrations").isDisabled>
    <cflocation url="#request.webRoot#/admin/settings/?disabledFeature=migrations" addtoken="false">
</cfif>
...
<cfif msgParam EQ "ran" AND runIDParam GT 0>
    ...
    <cfset statusModalBody = "Migration completed successfully. <a href='/admin/migrations/grad_migration_detail.cfm?runID=#runIDParam#'>View details</a>.">
<cfelseif msgParam EQ "ran_with_errors" AND runIDParam GT 0>
    ...
    <cfset statusModalBody = "Migration completed with errors.#(migrationErrCountParam GT 0 ? ' <strong>#migrationErrCountParam#</strong> record(s) reported errors.' : '')# <a href='/admin/migrations/grad_migration_detail.cfm?runID=#runIDParam#'>View details</a>.">
...
<cfset schedulerUrl = request.siteBaseUrl & "/admin/migrations/run_grad_migration.cfm?triggeredBy=scheduled&format=json">
...
<!--- Sort params for run history --->
<cfset allowedRunSortCols = ["GRADYEAR","STATUS","EXECUTEDAT"]>
<cfset sortCol = (structKeyExists(url, "sortCol") AND arrayFindNoCase(allowedRunSortCols, url.sortCol) GT 0) ? url.sortCol : "EXECUTEDAT">
<cfset sortDir = ((url.sortDir ?: "") EQ "ASC") ? "ASC" : "DESC">
<cfscript>
    arraySort(recentRuns, function(a, b) {
    ...
    function runSortLink(col) {
        var dir = (sortCol EQ col AND sortDir EQ "DESC") ? "ASC" : "DESC";
        return "?sortCol=" & col & "&sortDir=" & dir;
    }
    function runSortArrow(col) {
...
<div class="card mt-4 settings-shell settings-summary-card">
    <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <a href="/admin/migrations/grad_migration.cfm?preview=true&gradYear=#gradWindow.graduatingYear#"
           class="btn btn-ui-go btn-sm">
    ...
    <cfif showPreview AND structKeyExists(previewData, "success") AND previewData.success>
        <div class="card-body">
            ...
            <cfif previewData.totalStudents GT 0>
                ...
                <!--- Execute button --->
                <form method="post" action="/admin/migrations/run_grad_migration.cfm?force=true"
...
<div class="card mt-4 settings-shell settings-summary-card">
    ...
    <div class="card-body">
        <form method="post" action="/admin/migrations/save_grad_migration_settings.cfm">
    ...
    <div class="card-body">
        ...
        <div class="d-flex gap-2">
            ...
            <form method="post">
                ...
                <button type="submit" class="btn btn-ui-delete btn-sm">
            ...
            <div class="table-responsive">
                <table class="table table-sm table-hover align-middle mb-0 settings-table">
                    <thead>
                        <tr>
                            <th>Run</th>
                            <th><a href="#runSortLink('GRADYEAR')#" class="settings-sort-link">Grad Year#runSortArrow('GRADYEAR')#</a></th>
                            <th><a href="#runSortLink('STATUS')#" class="settings-sort-link">Status#runSortArrow('STATUS')#</a></th>
                            <th>Mode</th>
                            <th>Targeted</th>
                            <th>Migrated</th>
                            <th>Errors</th>
                            <th>Triggered By</th>
                            <th><a href="#runSortLink('EXECUTEDAT')#" class="settings-sort-link">Executed#runSortArrow('EXECUTEDAT')#</a></th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        <cfloop array="#recentRuns#" index="r">
                            <tr>
                                ...
                                <td>
                                    <a href="/admin/migrations/grad_migration_detail.cfm?runID=#r.RUNID#"
                                       class="btn btn-ui-go btn-sm" title="View Details">
                                        <i class="bi bi-eye"></i>
                                    </a>
                                    <cfif listFindNoCase("completed,completed_w_errors,failed", r.STATUS)>
                                        <form method="post" action="/admin/migrations/save_grad_migration_settings.cfm"
                                            ...
                                            <button type="submit" class="btn btn-ui-warning btn-sm" title="Rollback">
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-#statusModalClass#">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-ui-go" data-bs-dismiss="modal">OK</button>
                </div>
            </div>
        </div>
    </div>
