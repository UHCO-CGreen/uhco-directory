...
<cfif NOT detail.success>
    <cflocation url="/admin/user-review/?error=#urlEncodedFormat(detail.message)#" addtoken="false">
    <cfabort>
</cfif>
...
<cffunction name="renderReviewValue" access="private" returntype="string" output="false">
    ...
    <cfif arguments.fieldRow.SECTIONKEY EQ "contact" OR (arguments.fieldRow.SECTIONKEY EQ "general" AND arguments.fieldRow.FIELDNAME EQ "NameAliases")>
...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="/admin/user-review/">User Review</a></li>
...
<div class="card shadow-sm mb-4 settings-shell settings-summary-card">
    <div class="card-body">
        <form method="post" action="/admin/user-review/resolve.cfm" class="mb-0">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <button type="submit" name="action" value="approveAll" class="btn btn-ui-save"><i class="bi bi-check2-circle me-1"></i>Approve Entire Submission</button>
                <button type="submit" name="action" value="discardAll" class="btn btn-ui-delete" data-require-review-note><i class="bi bi-x-circle me-1"></i>Discard Entire Submission</button>
            </div>
        </form>
    </div>
</div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <cfif len(trim(fieldRow.CURRENTVALUE ?: ""))>
                                        <cfif fieldRow.SECTIONKEY EQ "contact" OR (fieldRow.SECTIONKEY EQ "general" AND fieldRow.FIELDNAME EQ "NameAliases")>
                                ...
                                <td class="text-end">
                                    <cfif NOT len(trim(fieldRow.RESOLUTION ?: ""))>
                                        <form method="post" action="/admin/user-review/resolve.cfm" class="mb-0 d-flex flex-column align-items-end gap-2">
                                            <input type="hidden" name="submissionID" value="#detail.submission.SUBMISSIONID#">
                                            ...
                                            <div class="d-flex justify-content-end gap-2">
                                                <button type="submit" name="action" value="approveField" class="btn btn-sm btn-ui-save">Approve</button>
                                                <button type="submit" name="action" value="discardField" class="btn btn-sm btn-ui-delete" data-require-review-note>Discard</button>
                                            </div>
                                        </form>
                                    </cfif>
                                </td>
