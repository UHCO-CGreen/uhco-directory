...
<!--- Pre-filter: keep only Staff --->
<cfset staffPreFiltered = []>
<cfloop from="1" to="#arrayLen(allUsers)#" index="i">
    ...
    <cfloop from="1" to="#arrayLen(uFlags)#" index="f">
        <cfif uFlags[f].FLAGNAME EQ "Staff">
            <cfset isStaff = true>
            <cfbreak>
        </cfif>
    </cfloop>
    <cfif isStaff>
        <cfset arrayAppend(staffPreFiltered, u)>
    </cfif>
</cfloop>
<cfset allUsers = staffPreFiltered>
...
<div class='d-flex justify-content-between mb-4'>
    <h1>Staff  <span class='badge bg-secondary fs-6'>#totalRecords#</span></h1>
...
<cfif arrayLen(pageRows) EQ 0>
    <cfset content &= "<tr><td colspan='6' class='text-center text-muted'>No staff found.</td></tr>">
</cfif>
...
<cfset content &= "<p class='text-muted small'>Showing #sliceStart#&##8211;#sliceEnd# of #totalRecords# staff</p>">
