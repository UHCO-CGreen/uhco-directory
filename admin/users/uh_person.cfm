...
<cfparam name="form.uhApiId" default="">
<cfparam name="form.createFirstName" default="">
...
<cfif uhApiId EQ "" AND len(trim(form.uhApiId))>
    <cfset uhApiId = trim(form.uhApiId)>
</cfif>
...
<cfif cgi.request_method EQ "POST" AND (isNumeric(form.applySourceUserID) OR len(trim(form.uhApiId)))>
    ...
    <cfif applyUserID LTE 0 AND len(trim(form.uhApiId))>
        ...
        <cfloop from="1" to="#arrayLen(existingUsers)#" index="u">
            <cfif compare(trim(existingUsers[u].UH_API_ID ?: ""), trim(form.uhApiId)) EQ 0>
                <cfset applyUserID = val(existingUsers[u].USERID ?: 0)>
                <cfset sourceUserID = toString(applyUserID)>
                <cfbreak>
            </cfif>
        </cfloop>
    </cfif>
    ...
    <cfif applyUserID LTE 0 AND len(trim(form.uhApiId)) AND len(trim(form.createFirstName)) AND len(trim(form.createLastName))>
        ...
        <cfset createResult = usersService.createUser({
            ...
            Title2 = "",
            Title3 = "",
            UH_API_ID = trim(form.uhApiId)
        })>

        <cfif structKeyExists(createResult, "success") AND createResult.success>
            <cfset applyUserID = val(createResult.userID ?: 0)>
            <cfset sourceUserID = toString(applyUserID)>
            <cfset saveMessage = "Created local user from UH API record.">
            <cfset saveMessageClass = "alert-success">
            
            <!--- Remove from staging table now that user is created --->
            <cfsilent>
                <cfset queryExecute(
    ...
    <cfif applyUserID LTE 0 AND saveMessage EQ "">
        <cfset saveMessage = "Unable to resolve or create a local user for this UH API record.">
        <cfset saveMessageClass = "alert-danger">
    </cfif>
        ...
        <cfif saveMessageClass NEQ "alert-danger">
            ...
            <cfelse>
                        ...
                        if (isStruct(arguments.node)) {
                            ...
                            for (i = 1; i <= arrayLen(keys); i++) {
                                ...
                                if (compareNoCase(k, arguments.keyName) EQ 0) {
                                    if (isSimpleValue(arguments.node[k])) {
                                        return toString(arguments.node[k] ?: "");
                                    }
                                    if (isBoolean(arguments.node[k])) {
                                        return arguments.node[k] ? "true" : "false";
                                    }
                                }
                            }
                ...
                <cfelse>
                    ...
                    <!--- Staff/Faculty excluded: UH API has no way to distinguish which of the 4 local
                          faculty flags (or Staff vs other roles) applies, so those must be set manually by an admin. --->
    ...
    <cfelseif applyUserID GT 0 AND len(trim(form.applyFlagName))>
        ...
        <cfif saveMessage EQ "" OR saveMessage EQ "Created local user from UH API record.">
            <cfset allFlagsResult = flagsService.getAllFlags()>
            <cfif structKeyExists(allFlagsResult, "success") AND allFlagsResult.success>
                <cfloop from="1" to="#arrayLen(allFlagsResult.data)#" index="f">
            ...
            <cfif flagID LTE 0>
                <cfset saveMessage = "Unable to sync flag: '#EncodeForHTML(requestedFlagName)#' was not found in UserFlags.">
                <cfset saveMessageClass = "alert-danger">
            </cfif>
        </cfif>
        ...
        <cfif saveMessageClass NEQ "alert-danger" AND saveMessageClass NEQ "alert-warning">
            ...
            <cfif targetHasFlag AND NOT userHasFlag>
                ...
                <cfif structKeyExists(actionResult, "success") AND actionResult.success>
                    <cfset saveMessage = "Added flag '" & requestedFlagName & "' from API value.">
                    <cfset saveMessageClass = "alert-success">
                <cfelse>
            ...
            <cfelseif NOT targetHasFlag AND userHasFlag>
                ...
                <cfif structKeyExists(actionResult, "success") AND actionResult.success>
                    <cfset saveMessage = "Removed flag '" & requestedFlagName & "' based on API value.">
                    <cfset saveMessageClass = "alert-success">
                <cfelse>
            ...
            <cfelse>
                <cfif saveMessage EQ "">
                    <cfset saveMessage = "No change needed for flag '" & requestedFlagName & "'.">
                    <cfset saveMessageClass = "alert-info">
                </cfif>
            </cfif>
        </cfif>
    <cfelseif applyUserID GT 0 AND len(trim(form.applyApiField))>
        ...
        <cfelse>
            ...
            <cfif saveMessageClass NEQ "alert-danger" AND saveMessageClass NEQ "alert-warning">
                ...
                <cfif structKeyExists(updateResult, "success") AND updateResult.success>
                    <cfset saveMessage = (saveMessage NEQ "" ? saveMessage & " " : "") & "Updated " & applyField & " from API value.">
                    <cfset saveMessageClass = "alert-success">
                <cfelse>
...
<cfelseif uhApiId NEQ "">
    ...
    <cfloop from="1" to="#arrayLen(allUsers)#" index="u">
        <cfif compare(trim(allUsers[u].UH_API_ID ?: ""), uhApiId) EQ 0>
            <cfset dbUser = allUsers[u]>
            <cfset dbUserFound = true>
            <cfset sourceUserID = toString(allUsers[u].USERID ?: "")>
            <cfbreak>
        </cfif>
    </cfloop>
</cfif>
...
<cfif uhApiId EQ "">
    <cfset content &= "
    <div class='alert alert-danger'>Missing UH API ID. Pass uhApiId in the URL.</div>
    ">
<cfelseif uhApiToken EQ "" OR uhApiSecret EQ "">
    <cfset content &= "
    <div class='alert alert-danger'>UH API credentials are not configured. Set environment variables UH_API_TOKEN and UH_API_SECRET.</div>
    <p><strong>UH_API_ID:</strong> #EncodeForHTML(uhApiId)#</p>
    ">
<cfelse>
    ...
    <cfset apiCreateFirstName = isStruct(apiPerson) ? trim(apiPerson.first_name ?: apiPerson.firstName ?: "") : "">
    <cfset apiCreateLastName = isStruct(apiPerson) ? trim(apiPerson.last_name ?: apiPerson.lastName ?: "") : "">
    <cfset canCreateLocalUser = (apiCreateFirstName NEQ "" AND apiCreateLastName NEQ "" AND uhApiId NEQ "")>

    <cfset content &= "
    <h4>Lookup</h4>
    <p><strong>UH_API_ID:</strong> #EncodeForHTML(uhApiId)#</p>
    <p><strong>Status:</strong> #EncodeForHTML(statusCode)#</p>
    ">

    <cfif left(statusCode, 3) NEQ "200">
        <cfset content &= "<div class='alert alert-danger'>UH API request failed.</div>">
    </cfif>
    ...
    <cfif left(statusCode, 3) EQ "200">
        <cfscript>
            ...
            function findValueByKeyDeep(any node="", required string keyName) {
                ...
                if (isStruct(arguments.node)) {
                    ...
                    // First pass: exact key match where value is simple (preferred).
                    for (i = 1; i <= arrayLen(keys); i++) {
                        k = keys[i];
                        if (compareNoCase(k, arguments.keyName) EQ 0) {
                            if (isSimpleValue(arguments.node[k]) AND len(trim(toString(arguments.node[k])))) {
                                return arguments.node[k];
                            }
                            if (isBoolean(arguments.node[k])) {
                                return arguments.node[k];
                            }
                    ...
                    // Second pass: recurse to find simple value in children.
                    for (i = 1; i <= arrayLen(keys); i++) {
                        found = findValueByKeyDeep(node=arguments.node[keys[i]], keyName=arguments.keyName);
                        if (isSimpleValue(found) AND len(trim(toString(found)))) {
                            return found;
                        }
                        if (isBoolean(found)) {
                            return found;
            ...
            function getApiValue(required any s, required string keyListCsv) {
                ...
                for (i = 1; i <= arrayLen(names); i++) {
                    ...
                    if (isSimpleValue(val) AND len(trim(toString(val)))) {
                        return toString(val);
                    }
            ...
            function getApiDirectValue(required struct s, required string keyListCsv) {
                ...
                for (i = 1; i <= arrayLen(names); i++) {
                    ...
                    for (j = 1; j <= arrayLen(keys); j++) {
                        if (compareNoCase(keys[j], nameToFind) EQ 0) {
                            return arguments.s[keys[j]];
                        }
                    }
                }
                return "";
            ...
            function normalizeForCompare(required string fieldName, required string v) {
                ...
                if (findNoCase("email", arguments.fieldName)) {
                    return out;
                }
                if (findNoCase("phone", arguments.fieldName)) {
                    return reReplace(arguments.v, "[^0-9]", "", "all");
                }
                return out;
            }
            ...
            function hasFlag(required array flags, required string flagName) {
                ...
                for (i = 1; i <= arrayLen(arguments.flags); i++) {
                    if (isStruct(arguments.flags[i])) {
                        ...
                        if (compareNoCase(fName, arguments.flagName) EQ 0) {
                            return true;
                        }
                    }
                }
            ...
            function toYesNo(required any v) {
                ...
                if (s EQ "true" OR s EQ "1" OR s EQ "yes" OR s EQ "y") {
                    return "Yes";
                }
                if (s EQ "false" OR s EQ "0" OR s EQ "no" OR s EQ "n") {
                    return "No";
                }
                return "N/A";
            }
        ...
        <cfset comparisonRows = [
            ...
            { label="Email (DB: EmailPrimary)", dbKey="EMAILPRIMARY", apiKeys="email,emailAddress", canUpdate=true },
        ...
        <cfloop from="1" to="#arrayLen(comparisonRows)#" index="r">
            ...
            <cfset showApplyButton = row.canUpdate AND hasApiValue AND ((NOT hasDbValue) OR valuesDiffer) AND (isNumeric(sourceUserID) OR canCreateLocalUser)>
            <cfset content &= "
                <tr>
        ...
        <cfloop from="1" to="#arrayLen(flagCompareRows)#" index="r">
            ...
            <cfset showFlagApplyButton = (isNumeric(sourceUserID) OR canCreateLocalUser) AND (apiDisplay EQ "Yes" OR apiDisplay EQ "No") AND apiDisplay NEQ dbDisplay>
        ...
        <cfloop from="1" to="#arrayLen(apiOnlyRows)#" index="r">
            <cfset row = apiOnlyRows[r]>
            <cfset apiVal = getApiValue(apiPerson, row.apiKeys)>
            <cfset apiDisplay = displayVal(apiVal)>
            <cfset content &= "
                <tr>
        ...
        <cfset content &= "
            ...
            ">
        <cfif NOT dbUserFound>
            <cfset content &= "<div class='alert alert-info mt-3'>No local user exists yet for this UH API record. Using any sync action will create the local user first.</div>">
        </cfif>
    </cfif>
