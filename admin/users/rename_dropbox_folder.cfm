...
<cftry>
    ...
    <cfset result = sourceService.renameUserDropboxFolder(
        firstName   = firstName,
        lastName    = lastName,
    ...
    <cfset responseData["fromPath"]       = result.fromPath>
    <cfset responseData["toPath"]         = result.toPath>
    <cfset responseData["publishCreated"] = result.publishCreated>

    <cfif result.success>
        <cfset usersService = createObject("component", "cfc.users_service").init()>
        <cfset usersService.updateDropboxFolderPath(targetUserID, result.toPath)>
    </cfif>
...
<cfcatch type="any">
    <cflog
        file="dropbox-folder-rename"
        type="error"
        text="rename_dropbox_folder failed | userID=#targetUserID# | message=#cfcatch.message# | detail=#cfcatch.detail#"
    >
