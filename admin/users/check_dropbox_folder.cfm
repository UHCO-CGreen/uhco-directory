...
<cftry>
    ...
    <cfset result = sourceService.checkUserDropboxFolder(
        firstName   = firstName,
        lastName    = lastName,
    ...
    <cfset responseData["provider"]           = result.provider>
    ...
    <cfset responseData["exists"]             = result.exists>
    <cfset responseData["publishExists"]      = result.publishExists>
    <cfset responseData["legacyExists"]       = result.legacyExists>
    <cfset responseData["legacyFolder"]       = result.legacyFolder>
    <cfset responseData["legacyFolderPath"]   = result.legacyFolderPath>
    <cfset responseData["legacyParentFolder"] = result.legacyParentFolder>

    <cfif result.success AND result.exists>
...
<cfcatch type="any">
    <cflog
        file="dropbox-folder-check"
        type="error"
        text="check_dropbox_folder failed | userID=#targetUserID# | message=#cfcatch.message# | detail=#cfcatch.detail#"
    >
