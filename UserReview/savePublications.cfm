...
<cftry>
    ...
    <cfset result = userReviewService.saveLivePublications(
        actor = userReviewAuth.getSessionUser(),
        formScope = form
    )>
    <cfset emitUserReviewResult(result)>
<cfcatch type="any">
    <cflog file="userreview-publications" type="error" text="savePublications failed | message=#cfcatch.message# | detail=#cfcatch.detail#">
    <cfset emitUserReviewResult({ success = false, statusCode = 500, message = "Live publications update failed.", errors = [] })>
</cfcatch>
</cftry>